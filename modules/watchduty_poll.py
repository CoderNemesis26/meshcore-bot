...
Used by WatchDutyService and fire/evac commands: feed channel (summary when acreage/containment change) and report channel.
"""
...
def watchduty_bbox_from_config(config: Any) -> Optional[Tuple[float, float, float, float]]:
    """Parse [WatchDuty_Service] bbox (lat_min,lng_min,lat_max,lng_max) or return None."""
    if not getattr(config, "has_section", lambda _: False)("WatchDuty_Service"):
        return None
    bbox_str = config.get("WatchDuty_Service", "bbox", fallback="").strip()
    if not bbox_str:
        return None
    try:
...
def watchduty_min_acres_from_config(config: Any) -> float:
    """Minimum acres from [WatchDuty_Service] min_acres (default 1.0)."""
    if not getattr(config, "has_section", lambda _: False)("WatchDuty_Service"):
        return 1.0
    try:
        return float(config.getfloat("WatchDuty_Service", "min_acres", fallback=1.0))
    except Exception:
        return 1.0
...
def fetch_active_geo_events_for_user_query(
    ...
    *,
    ...
    Uses the same bbox and min_acres as [WatchDuty_Service] when that section exists.
    """
